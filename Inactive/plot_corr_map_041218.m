clear all
% -----To be adjusted-----
Data=load ('M:\AllProcessedFluoData\20180322_182730-GCaMP6s_after_2.5eth.mat');%the directory of the file to be processed
...
if weird_plane==1
    ...
    z_stepsize=5;
    Distancethre=10;%if the minimal distance between a cell in suspected plane and any other cell in plane above or below is further than this value, that cell in suspected plane will be discarded.
    ----------------------------------
    for i6=1:length(suspectz)
        ...
        Allfluo(disc_list2,:)=[];
        spPos=Allpos;
        fluorescence_time_series=Allfluo;
        Sc=Data.Sc;
        if use_dFF_For_Cor_Map==1
            AlldFF(disc_list,:)=[];
        end
    ...
    save('20180322_182730-GCaMP6s_after_2.5eth-no_werid_cells.mat','spPos','fluorescence_time_series','Sc');
end
