...
list_of_directories = {...
%                 'J:\Cui\ProcessedData 051718-052118\dFF Computed Data'...
%                 'J:\Cui\ProcessedData 042318-051018\dFF Computed Data'...
                'J:\Cui\ProcessedData 051718-052118\dFF Computed Data\0517F1T1'...
%                 'J:\Cui\ProcessedData 053118(Tidal Neural Activity)\dFFComputedData'...
%                 'E:\Dawnis\IndependenceDay\L07\SptSTructural\_20170704_131820'...
    };

stack_frequency=1.28;
window_size=99999;
basepercent=5;
lowpass_on=0;
...
for directory_idx  = 1:numel(list_of_directories)
    ...
    for fileseq=1:size(DataList,1)
        %--------------Load data-------------------
        CurrentFileName=DataList(fileseq).name
        CurrentBGFile=BGList(fileseq).name
        
        Data=load(strcat(list_of_directories{directory_idx},'\',CurrentFileName));
        fts=Data.fluorescence_time_series;
        BGFileContent=csvread(strcat(list_of_directories{directory_idx},'\',CurrentBGFile),1,1);
        BGfts=BGFileContent(:,1);
        %-----------------Subtract BG---------------
        
        ExpandedBGfts=repmat(BGfts',size(fts,1),1);
        BGSubtractedFTS=fts-ExpandedBGfts;
        
        %----------------Compute dFF------------------
        print='start computing...'
        dFF = lsExplorer_Compute_dFF(BGSubtractedFTS, stack_frequency, window_size, basepercent, lowpass_on);
        dFF(any(isnan(dFF),2),:)=0;
        print='Computation Done!'
        %-----------------Save computed files-----------------
        filename=CurrentFileName;
        path=list_of_directories{directory_idx};
        
        FluoAfterSubtraction=BGSubtractedFTS;
        BackGroundFTS=BGfts';
            
        MovingWindowSize=window_size;
        StackFrequency = stack_frequency;
        BasePercent = basepercent; 
        LowPass_on_Check =lowpass_on;
        
        savepath = strcat(path,'\',filename);
    ...
    disp(sprintf('Finished %s !',list_of_directories{directory_idx}));
%     clear BGfts
end
print3='All Done!'
